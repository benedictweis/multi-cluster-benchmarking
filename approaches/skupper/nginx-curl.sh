...
kubectl create namespace nginx-curl --context "$CLUSTER_1_CONTEXT" --dry-run=client -o yaml | kubectl apply --context "$CLUSTER_1_CONTEXT" -f -
kubectl create namespace nginx-curl --context "$CLUSTER_2_CONTEXT" --dry-run=client -o yaml | kubectl apply --context "$CLUSTER_2_CONTEXT" -f -
kubectl config set-context "$CLUSTER_1_CONTEXT" --namespace nginx-curl
kubectl config set-context "$CLUSTER_2_CONTEXT" --namespace nginx-curl
...
approachinfo "Exposing nginx-server"
skupper expose deployment/nginx-server --port 80 --context "$CLUSTER_1_CONTEXT"
